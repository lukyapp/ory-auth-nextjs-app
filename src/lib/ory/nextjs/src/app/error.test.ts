...
describe('getError', () => {
  ...
  describe('when search params contain an id', () => {
    test('fetches the flow error from the SDK', async () => {
      ...
      (serverSideFrontendClient().getFlowError as jest.Mock).mockResolvedValue(flowError);
    ...
    test('returns unknown_error with default message when SDK call throws a non-Error', async () => {
      (serverSideFrontendClient().getFlowError as jest.Mock).mockRejectedValue('some string error');
