...
const createMockRequest = (url: string, options: Partial<NextRequest> = {}): NextRequest => {
  return {
...
describe('proxyRequest', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    process.env['NEXT_PUBLIC_ORY_SDK_URL'] = 'https://playground.projects.oryapis.com';
  ...
  it('proxies a request and modifies the set-cookie header', async () => {
    ...
    const upstreamResponseHeaders = new Headers({
      'set-cookie': 'session=a; Domain=playground.projects.oryapis.com; Path=/; HttpOnly',
  ...
  it('proxies a request and modifies the HTML', async () => {
    ...
    const body = await response?.text();
    expect(body).toEqual("<html><a href='http://localhost/self-service/logout'>logout</a></html>");
  });
  ...
  it('modifies location header for redirects', async () => {
    ...
    expect(response?.headers.get('location')).toBe('http://localhost/self-service/login');
  ...
  it('modifies relative location header for redirects', async () => {
    ...
    expect(response?.headers.get('location')).toBe('http://localhost/custom-redirect');
    expect(response?.status).toBe(302);
  });
  ...
  it.each([
    ...
    async ({ project, path, expect: expectUrl }) => {
      ...
      expect(response?.headers.get('location')).toBe('http://localhost' + expectUrl);
      expect(response?.status).toBe(302);
    },
  );
  ...
  it('bypasses requests that do not match proxy paths', async () => {
    ...
    const body = "<html><a href='http://localhost/self-service/logout'>logout</a></html>";
  ...
  it('preserves additional forwarded headers', async () => {
    ...
    const fetch = jest.fn().mockResolvedValue(
      new Response('https://playground.projects.oryapis.com/self-service/login', {
        status: 200,
      }),
    );
    global.fetch = fetch;
