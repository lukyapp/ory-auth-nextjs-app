...
describe('useSession', () => {
  ...
  it('fetches and sets session successfully', async () => {
    ...
    await waitFor(() => expect(screen.getByText(`Session: ${mockSession.id}`)).toBeInTheDocument());
  ...
  it('fetches session automatically if not provided', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Session: test-session-id')).toBeInTheDocument());
