...
describe('test config provider', () => {
  it('should be possible to pass SDK url', () => {
    ...
    const { result } = renderHook(() => useOryConfiguration(), {
      wrapper: ({ children }) => (
        <OryConfigurationProvider sdk={{ url: sdkUrl }} project={{ name: 'test' }}>
          {children}
  ...
  describe('browser env', () => {
    it('should guess browser env', () => {
      const { result } = renderHook(() => useOryConfiguration(), {
        wrapper: ({ children }) => (
          <OryConfigurationProvider project={{ name: 'test' }}>{children}</OryConfigurationProvider>
        ),
      });
      ...
      for (const [variable, value] of [
        ...
        describe('with ' + variable + '=' + value, () => {
          ...
          it('should throw if unset', () => {
            ...
            expect(() => render(<OryConfigurationProvider project={{ name: 'test' }} />)).toThrow(
