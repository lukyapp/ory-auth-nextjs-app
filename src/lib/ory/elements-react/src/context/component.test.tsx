...
describe('defaultNodeSorter', () => {
  it('correctly sorts nodes from captcha form response', () => {
    ...
    const nodeOrder = sortedNodes.map((node) => ({
      ...
      inputType: isUiNodeInputAttributes(node.attributes) ? node.attributes.type : undefined,
      name: isUiNodeInputAttributes(node.attributes) ? node.attributes.name : undefined,
    }));
    ...
    if (captchaNodes.length > 0 && submitButtons.length > 0) {
      const lastCaptchaIndex = sortedNodes.indexOf(captchaNodes[captchaNodes.length - 1]);
  ...
  it('correctly sorts nodes from one-step registration with captcha', () => {
    ...
    // Snapshot test for consistent ordering
    expect(nodeOrder).toMatchSnapshot('one-step registration with captcha node order');
  ...
  it('correctly sorts nodes from two-step registration with captcha', () => {
    ...
    // Snapshot test for consistent ordering
    expect(nodeOrder).toMatchSnapshot('two-step registration with captcha node order');
