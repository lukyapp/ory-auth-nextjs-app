...
import { AuthenticatorAssuranceLevel, FlowType, UiContainer, UiNode } from '@ory/client-fetch';
...
test('should announce the second factor code by SMS for an sms address', () => {
  const { result } = renderHook(
    () => useCardHeaderText(container([addressNode('sms', '+4746788587')]), aal2CodeActiveOpts),
    { wrapper },
  );

  expect(result.current.description).toBe('A verification code will be sent by SMS');
});

test('should announce the second factor code by email for an email address', () => {
  const { result } = renderHook(
    () =>
      useCardHeaderText(container([addressNode('email', 'user@example.com')]), aal2CodeActiveOpts),
    { wrapper },
  );

  expect(result.current.description).toBe('A verification code will be sent by email');
});

test('should announce the second factor code by email when the channel is unknown', () => {
  const { result } = renderHook(() => useCardHeaderText(container([]), aal2CodeActiveOpts), {
    wrapper,
  });
...
test('should keep the email sent message on a wrong-code retry with an email address', () => {
  const { result } = renderHook(
    () =>
      useCardHeaderText(
        container([codeInputNode(), verificationResendNode('user@example.com')]),
        verificationOpts,
      ),
    { wrapper },
  );
...
test('should mention phone numbers in the subtitle when the input is labeled email or phone', () => {
  const { result } = renderHook(
    () =>
      useCardHeaderText(
        container([verificationAddressInputNode(1070018, 'Email or phone number')]),
        verificationOpts,
      ),
    { wrapper },
  );
