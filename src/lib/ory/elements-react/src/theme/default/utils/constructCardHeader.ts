  ...
  switch (opts.flowType) {
    ...
    case FlowType.Verification: {
      if (nodes.find((node) => 'name' in node.attributes && node.attributes.name === 'code')) {
        ...
        // the SMS wording.
        const sentViaSms =
          container.messages?.some((m) => m.id === 1080004) || findCodeChannel(nodes) === 'sms';
        return {
          title: intl.formatMessage({
            id: 'verification.title',
            defaultMessage: 'Verify your account',
          }),
          description: intl.formatMessage(kratosMessages[sentViaSms ? 1080004 : 1080003]),
      ...
      const acceptsPhone = nodes.some((node) => node.meta?.label?.id === 1070018);
      return {
        title: intl.formatMessage({
          id: 'verification.title',
          defaultMessage: 'Verify your account',
    ...
    case FlowType.Login: {
      ...
      if (opts.flow.refresh) {
        ...
        const description = codeSent
          ...
          : opts.formState?.current === 'method_active'
            ? intl.formatMessage(
                opts.formState.method === 'code' && findCodeChannel(nodes) === 'sms'
