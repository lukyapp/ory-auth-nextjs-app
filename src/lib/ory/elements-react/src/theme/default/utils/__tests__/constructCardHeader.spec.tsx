...
import { FlowType, UiNode, UiNodeGroupEnum, UiTextTypeEnum } from '@ory/client-fetch';
...
const wrapper =
  (lang: string) =>
  ({ children }: PropsWithChildren) => <IntlProvider locale={lang}>{children}</IntlProvider>;
  ...
  describe('messageId on refresh and aal2 login flows', () => {
    ...
    test('should set messageId to 1010025 on refresh login when the code was sent', () => {
      ...
      expect(res.result.current).toEqual({
        ...
        description: "A code was sent to your address. If you didn't receive it, please try again.",
        messageId: '1010025',
      });
    });
