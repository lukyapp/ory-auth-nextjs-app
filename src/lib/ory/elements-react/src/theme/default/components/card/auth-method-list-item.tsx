...
export function DefaultAuthMethodListItem({
  ...
  if (group === 'passkey') {
    ...
    return (
      <PasskeyListItem passkeyNode={passkeyNode} group={group} title={title} disabled={disabled} />
    );
  }
...
function PasskeyListItem({ group, title, passkeyNode, disabled }: PasskeyListItemProps) {
  const intl = useIntl();
  const Icon = iconsMap[group] || null;
