...
const getRemoveButtons = (nodes: UiNode[]): UiNodeInput[] =>
  nodes
    .filter((node) => 'name' in node.attributes && node.attributes.name === 'deviceauthn_remove')
...
export function OrySettingsDeviceauthn({ nodes }: HeadlessSettingsDeviceauthnProps) {
    ...
    <>
      ...
      >
        <Form.DeviceauthnSettings
          ...
          removeButtons={removeButtons.map((node) => {
            const onClick = removeDeviceauthnKeyHandler(node.attributes.value as string);
            return {
              ...node,
              onClick,
              buttonProps: {
      ...
      <Card.SettingsSectionFooter
        text={intl.formatMessage({
          ...
          defaultMessage: 'Trusted devices are used to authenticate with registered mobile devices',
        })}
      ></Card.SettingsSectionFooter>
    </>
  );
