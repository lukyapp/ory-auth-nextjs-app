...
import { FlowType, UiNode, UiNodeGroupEnum, UiNodeInputAttributes } from '@ory/client-fetch';
import { OryFlowContainer } from '../../util';
import { computeDefaultValues, getLoginHint, resolveLoginHint, unrollTrait } from './form-helpers';

function flowContainer(
  flowType: FlowType,
  { requestUrl, oidcLoginHint }: { requestUrl?: string; oidcLoginHint?: string } = {},
): OryFlowContainer {
  return {
    flowType,
    flow: {
      ...
      oauth2_login_request:
        oidcLoginHint !== undefined ? { oidc_context: { login_hint: oidcLoginHint } } : undefined,
    },
...
describe('computeDefaultValues login_hint pre-fill', () => {
  ...
  test('should fall through to the next preference when the preferred field is filled', () => {
    const flow = {
      ui: {
        nodes: [inputNode('identifier', 'typed@example.com'), inputNode('traits.email', '')],
      },
    };
...
describe('getLoginHint', () => {
  ...
  test('should decode URL-encoded values', () => {
    expect(getLoginHint('?login_hint=jane%40example.com')).toBe('jane@example.com');
  });
  ...
  test('should trim surrounding whitespace', () => {
    expect(getLoginHint('?login_hint=%20jane@example.com%20')).toBe('jane@example.com');
  });
});
...
describe('resolveLoginHint', () => {
  test("should return the hint from the flow's request_url", () => {
    const container = flowContainer(FlowType.Login, {
      requestUrl: 'https://ory.example/self-service/login/browser?login_hint=url@example.com',
    });
  ...
  test('should decode a URL-encoded hint in the request_url', () => {
    const container = flowContainer(FlowType.Login, {
      requestUrl: 'https://ory.example/self-service/login/browser?login_hint=url%40example.com',
    });
    expect(resolveLoginHint(container)).toBe('url@example.com');
  });
  ...
  test('should ignore hints on non-login/registration flows', () => {
    const container = flowContainer(FlowType.Recovery, {
      requestUrl: 'https://ory.example/self-service/recovery/browser?login_hint=url@example.com',
      oidcLoginHint: 'oidc@example.com',
    });
  ...
  test('should return undefined when the request_url query has no hint', () => {
    const container = flowContainer(FlowType.Login, {
      requestUrl: 'https://ory.example/self-service/login/browser?return_to=/foo',
    });
    expect(resolveLoginHint(container)).toBeUndefined();
  });
