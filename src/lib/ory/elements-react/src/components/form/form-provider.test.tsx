...
function IdentifierProbe() {
  ...
  return <span data-testid="identifier">{String(getValues('identifier') ?? '')}</span>;
}
...
test("should pre-fill the identifier from the login_hint on the flow's request_url", () => {
  renderProvider(
    loginFlowWithIdentifier({
      requestUrl: 'https://ory.example/self-service/login/browser?login_hint=jane@example.com',
    }),
  );
