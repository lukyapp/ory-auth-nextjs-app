...
describe('onSubmitRegistration', () => {
  test('should fire registration success event before redirect on successful registration', async () => {
    ...
    await onSubmitRegistration(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
      body: {
        method: 'password',
  ...
  test('should fire validation error event on validation failure', async () => {
    ...
    const errorFlow = {
      ...
      ui: {
        ...
        nodes: [
          {
            messages: [{ id: 4000002, text: 'Password too weak', type: 'error' }],
            attributes: {},
            type: 'input',
            group: 'default',
          },
        ],
        messages: [{ id: 4000001, text: 'Email already exists', type: 'error' }],
      },
    };
  ...
  test('should not throw when callbacks are omitted', async () => {
    ...
    await expect(
      onSubmitRegistration(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
        body: {
          method: 'password',
          traits: { email: 'a@b.com' },
          password: 'secret',
