...
describe('Error identification utilities', () => {
  describe('isGenericErrorResponse', () => {
    ...
    it('should return false for invalid responses', () => {
      ...
      expect(isGenericErrorResponse({ error: { message: 'Error' } })).toBe(false);
    });
  });
  ...
  describe('Error type identification', () => {
    ...
    it('should identify needs privileged session error', () => {
      ...
      expect(isNeedsPrivilegedSessionError(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify self service flow expired error', () => {
      ...
      expect(isSelfServiceFlowExpiredError(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify self service flow disabled error', () => {
      ...
      expect(isSelfServiceFlowDisabled(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify browser location change required error', () => {
      ...
      expect(isBrowserLocationChangeRequired(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify self service flow replaced error', () => {
      ...
      expect(isSelfServiceFlowReplaced(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify session already available error', () => {
      ...
      expect(isSessionAlreadyAvailable(createErrorResponse('other'))).toBe(false);
    });
    ...
    it('should identify redirect URL not allowed error', () => {
      const response = createErrorResponse('self_service_flow_return_to_forbidden');
    ...
    it('should identify security identity mismatch error', () => {
      ...
      expect(isSecurityIdentityMismatch(createErrorResponse('other'))).toBe(false);
    });
  });

  describe('Response and Fetch error identification', () => {
    it('should identify ResponseError instances', () => {
      const responseError = new ResponseError({} as unknown as Response, 'Response error');
