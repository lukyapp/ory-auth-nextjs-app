...
interface MockResponse extends Omit<Response, 'clone' | 'json' | 'text' | 'headers'> {
  headers: {
    get: jest.Mock;
    entries: jest.Mock;
  };
...
describe('handleFlowError', () => {
  ...
  it('should handle address not verified error with continue_with flow URL', async () => {
    ...
    expect(opts.onRedirect).toHaveBeenCalledWith('https://example.com/verification', true);
  });
  ...
  it('should handle browser location change required error', async () => {
    ...
    expect(opts.onRedirect).toHaveBeenCalledWith('https://example.com/redirect', true);
  });
  ...
  it('should handle needs privileged session error', async () => {
    ...
    expect(opts.onRedirect).toHaveBeenCalledWith('https://example.com/login', true);
  });
  ...
  it('should handle text/html response', async () => {
    ...
    const responseError = new ResponseError(mockResponse as unknown as Response, 'HTML error');
    const handler = handleFlowError(opts);
  ...
  it('should handle unknown content type', async () => {
    ...
    const responseError = new ResponseError(mockResponse as unknown as Response, 'Binary data');
    const handler = handleFlowError(opts);
...
function makeResponseError(status: number, body: unknown, contentType = 'application/json') {
...
describe('toBody', () => {
  ...
  it('should throw error for invalid JSON', async () => {
    ...
    await expect(toBody(mockResponse)).rejects.toThrow('Unable to decode API response using JSON');
