...
describe('onSubmitVerification', () => {
  test('should fire verification success event on successful submission', async () => {
    ...
    await onSubmitVerification(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
  ...
  test('should fire validation error event on validation failure', async () => {
    ...
    const errorFlow = {
      ...
      ui: {
        ...
        nodes: [
          {
            messages: [{ id: 4000002, text: 'Code is required', type: 'error' }],
            attributes: {},
            type: 'input',
            group: 'default',
          },
  ...
  test('should not throw when callbacks are omitted', async () => {
    ...
    await expect(
      onSubmitVerification(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
        body: { method: 'code', code: '123456' } as any,
        onRedirect,
        setFlowContainer,
      }),
