...
describe('useFunctionalNodes', () => {
  test('returns only functional nodes', () => {
    ...
    expect(result.current.every((node: UiNode) => functionalGroups.includes(node.group))).toBe(
      true,
    );
    ...
    expect(
      result.current.some(
        (node) => node.group === UiNodeGroupEnum.Password || node.group === UiNodeGroupEnum.Oidc,
      ),
    ).toBe(false);
  });
});
...
describe('SSO node functions', () => {
  ...
  test('hasSingleSignOnNodes returns true when SSO nodes exist', () => {
    const nodes = [createTestNode(UiNodeGroupEnum.Password), createTestNode(UiNodeGroupEnum.Oidc)];
  ...
  test('withoutSingleSignOnNodes filters out SSO nodes', () => {
    ...
    expect(filtered.some((node) => node.group === UiNodeGroupEnum.Oidc)).toBe(false);
    expect(filtered.some((node) => node.group === UiNodeGroupEnum.Saml)).toBe(false);
    expect(filtered.some((node) => node.group === UiNodeGroupEnum.Password)).toBe(true);
    expect(filtered.some((node) => node.group === UiNodeGroupEnum.Default)).toBe(true);
  });
});
...
describe('useNodeGroupsWithVisibleNodes', () => {
  test('returns groups with at least one visible node', () => {
    ...
    const { result } = renderHook(() => useNodeGroupsWithVisibleNodes(mixedNodes));
...
describe('utils/ui', () => {
  test('useNodesGroups without omit', () => {
    const { result } = renderHook(() => useNodesGroups(allMethodsInitialForm.ui.nodes as UiNode[]));
