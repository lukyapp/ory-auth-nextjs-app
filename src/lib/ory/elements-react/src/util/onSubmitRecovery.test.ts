...
describe('onSubmitRecovery', () => {
  test('should fire recovery success event on successful submission', async () => {
    ...
    await onSubmitRecovery(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
  ...
  test('should fire validation error event on validation failure', async () => {
    ...
    const errorFlow = {
      ...
      ui: {
        ...
        nodes: [
          {
            messages: [{ id: 4000002, text: 'Email is required', type: 'error' }],
            attributes: {},
            type: 'input',
            group: 'default',
          },
  ...
  test('should not throw when callbacks are omitted', async () => {
    ...
    await expect(
      onSubmitRecovery(mockFlow, mockConfig as unknown as OryElementsConfiguration, {
        body: { method: 'code', email: 'user@example.com' } as any,
        onRedirect,
        setFlowContainer,
      }),
