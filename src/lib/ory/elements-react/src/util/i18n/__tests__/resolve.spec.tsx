...
const Resolvers = ({ uiText }: { uiText: UiText }) => {
  ...
  return (
    <div>
      ...
      <span data-testid="option">{resolveOptionLabel('traits.country', 'DE', intl)}</span>
    </div>
  );
};
...
describe('optional forms.* translations', () => {
  ...
  test('should not log MISSING_TRANSLATION errors when no custom translation is defined', () => {
    ...
    expect(screen.getByTestId('placeholder').textContent).toBe('Voer jou E-Mail in');
  ...
  test('should use custom translations when they are defined', () => {
    ...
    expect(screen.getByTestId('placeholder').textContent).toBe('Aangepaste plekhouer');
