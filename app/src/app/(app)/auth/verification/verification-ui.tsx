...
import { getNodeLabel, type UiText, type VerificationFlow } from '@ory/client-fetch';
import {
  OryLocales,
...
import { DefaultCardFooter, Verification } from '@ory/elements-react/theme';
import { AlertCircle, BadgeCheck, CheckCircle2, ChevronDown, Loader2 } from 'lucide-react';
import {
  createContext,
  useContext,
  useEffect,
...
type VerificationUiProps = {
  clientName?: string | null;
  config: OryClientConfiguration;
  flow: VerificationFlow;
};

export function VerificationUi({ clientName, config, flow }: VerificationUiProps) {
  return (
    <VerificationClientNameContext.Provider value={clientName?.trim() || null}>
      <Verification flow={flow} config={config} components={squareVerificationComponents} />
    </VerificationClientNameContext.Provider>
  );
}

const VerificationClientNameContext = createContext<string | null>(null);

function SquareCardRoot({ children }: PropsWithChildren) {
  const intl = useIntl();
  const clientName =
    useVerificationClientName() ??
    intl.formatMessage({
    ...
    <div className="fixed inset-0 min-h-screen overflow-y-auto overscroll-contain bg-white text-[#1f1f1f]">
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[33rem_1fr]">
        <aside className="flex min-h-[32rem] flex-col bg-[#f3f3f3] px-8 py-8 sm:px-12 lg:min-h-screen lg:px-20 lg:py-20">
          ...
          <div className="flex flex-1 flex-col justify-center py-12 lg:py-0">
            <div className="mb-7 grid h-24 w-24 place-items-center rounded-[1.25rem] bg-[#dddddd] shadow-[0_2px_8px_rgba(0,0,0,0.10)]">
              <BadgeCheck className="h-12 w-12 stroke-[2.5]" aria-hidden="true" />
            </div>
            <h1 className="max-w-[24rem] text-[1.75rem] leading-[1.22] font-bold tracking-normal text-balance sm:text-[2rem]">
              {intl.formatMessage({
                ...
                id: 'verification.hero.title',
              })}
            </h1>
            <p className="mt-7 max-w-[24rem] text-lg leading-7 font-semibold text-[#777]">
              {intl.formatMessage({
                defaultMessage: 'Confirm your email address to keep your account secure.',
                id: 'verification.hero.description',
              })}
            </p>
          </div>
        ...
        <main className="flex items-center justify-center px-8 py-14 sm:px-12 lg:px-20">
          <section className="w-full max-w-[30rem]" aria-labelledby="verification-form-title">
            {children}
          </section>
        </main>
      </div>
...
function SquareCodeInput({ inputProps }: OryNodeInputProps) {
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    const value = normalizeVerificationCode(event.currentTarget.value, maxLength);
  ...
  const handlePaste = (event: ClipboardEvent<HTMLInputElement>) => {
    const value = normalizeVerificationCode(event.clipboardData.getData('text/plain'), maxLength);
  ...
  return (
    <input
      ...
      onPaste={handlePaste}
      pattern="[0-9]*"
      value={normalizeVerificationCode(String(inputProps.value ?? ''), maxLength)}
      className="h-12 w-full rounded-md border border-[#d8d8d8] bg-white px-4 text-base text-[#242424] transition placeholder:text-[#8a8a8a] hover:border-[#bdbdbd] focus:border-[#1f5ed8] focus:ring-0 focus:outline-none disabled:cursor-not-allowed disabled:bg-[#f3f3f3] disabled:text-[#777]"
    />
  );
}
...
const squareVerificationComponents: OryFlowComponentOverrides = {
  Card: {
...
function useVerificationClientName() {
  return useContext(VerificationClientNameContext);
}
...
function normalizeVerificationCode(value: string, maxLength: number) {
